...
const SightingById = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: response } = await axios.get(`http://localhost:5001/api/unverifiedsightings/getSightingsById/` + sightingId);
  ...
  return (
    sighting && (<div>
      ...
      <div className='date'> {new Date(sighting.date).toLocaleDateString()}</div>
